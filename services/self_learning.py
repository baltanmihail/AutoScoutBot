...
class SelfLearningEngine:
    ...
    def __init__(self, db_path: str = "query_history.db", min_samples: int = 5):
        self.db_path = db_path
        self.min_samples = min_samples  # Минимум запросов для выявления паттерна
    ...
    def _discover_patterns(self, cursor) -> List[Dict]:
        ...
        # Находим успешные запросы (AI relevance > 80)
        cursor.execute("""
            SELECT 
                q.query_text,
                r.cluster,
    ...
    def _save_few_shot_examples(self, examples: List[Dict]):
        ...
        try:
            output_file = "services/learned_examples.py"
    ...
    def _generate_recommendations(self, total_queries: int, 
        ...
        if total_queries >= 100:
            recommendations.append(
                ...
                "Запустите: python export_for_finetuning.py"
            )
        
        return recommendations
    
    def export_for_finetuning(self, output_file: str = "finetuning_dataset.jsonl") -> int:
        """
        ...
        try:
            ...
            # Экспортируем только качественные примеры (AI relevance > 70)
            cursor.execute("""
                SELECT 
                    q.query_text,
                    r.startup_name,
            ...
            if len(rows) < 100:
                logger.warning(f"⚠️ Недостаточно данных для fine-tuning: {len(rows)}/100")
                return 0
