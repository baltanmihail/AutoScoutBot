...
class ContinuousLearner:
    ...
    - CONTINUOUS_LEARNING['queries_threshold']
    - CONTINUOUS_LEARNING['hours_interval']
    """
    
    def __init__(self, queries_threshold: int = None, hours_interval: int = None):
        # Загружаем настройки из config.py
        try:
            from config import CONTINUOUS_LEARNING, SELF_LEARNING
            self.queries_threshold = queries_threshold or CONTINUOUS_LEARNING.get('queries_threshold', 20)
            self.hours_interval = hours_interval or CONTINUOUS_LEARNING.get('hours_interval', 24)
            min_samples = SELF_LEARNING.get('min_samples', 3)
        except ImportError:
            self.queries_threshold = queries_threshold or 20
            self.hours_interval = hours_interval or 24
            min_samples = 3
        ...
        self.thread = None
        self.engine = SelfLearningEngine(min_samples=min_samples)
    ...
    def notify_new_query(self):
        ...
        # Проверяем, пора ли обучаться (глубокое обучение)
        if self.queries_since_training >= self.queries_threshold:
            logger.info(f"🧠🔄 Накоплено {self.queries_since_training} запросов. Запуск ГЛУБОКОГО обучения...")
            self._train()
    ...
    def _train(self):
        """Запуск глубокого обучения"""
        try:
            report = self.engine.analyze_and_learn()
            
            logger.info(f"📊 ГЛУБОКОЕ обучение завершено: "
...
def get_continuous_learner() -> ContinuousLearner:
    """Получить singleton экземпляр (настройки из config.py)"""
    global _continuous_learner
    if _continuous_learner is None:
        _continuous_learner = ContinuousLearner()  # Настройки из config.py
